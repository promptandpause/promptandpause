-- ============================================================================
-- Fix ALL Email Templates - Remove Duplicate Logo, Fix Header, Update Daily Reminder
-- ============================================================================
-- This script updates all email templates with clean HTML structure
-- Run this in Supabase SQL Editor
-- ============================================================================

-- 1. Update daily_reminder template to match the new template structure
...
SET
  ...
  html_body = '<div style="margin-bottom: 24px;"><p style="color: #374151; font-size: 16px; line-height: 24px; margin: 0 0 16px 0;">Hi {{userName}},</p><p style="color: #374151; font-size: 16px; line-height: 24px; margin: 0;">Here is your daily reminder for {{reminderDate}}.</p></div>',
...
-- 2. Update ALL templates with clean HTML (no logo in html_body, proper spacing)
-- The logo will be rendered by the email wrapper, not in the template body

UPDATE email_templates SET html_body = '<div style="margin-bottom: 24px;"><p style="color: #374151; font-size: 16px; line-height: 24px; margin: 0 0 16px 0;">Welcome to Prompt & Pause, {{userName}}.</p><p style="color: #374151; font-size: 16px; line-height: 24px; margin: 0;">You can start whenever you''re ready.</p></div>' WHERE template_key = 'welcome';

UPDATE email_templates SET html_body = '<div style="margin-bottom: 24px;"><p style="color: #374151; font-size: 16px; line-height: 24px; margin: 0 0 16px 0;">Hi {{userName}},</p><p style="color: #374151; font-size: 16px; line-height: 24px; margin: 0;">Your {{billingCycle}} subscription for {{amount}} is confirmed.</p></div>' WHERE template_key = 'subscription_confirmation';
...
UPDATE email_templates SET html_body = '<div style="margin-bottom: 24px;"><p style="color: #374151; font-size: 16px; line-height: 24px; margin: 0 0 16px 0;">Hi {{userName}},</p><p style="color: #374151; font-size: 16px; line-height: 24px; margin: 0;">Your trial expired on {{trialEndDate}}.</p><p style="color: #374151; font-size: 16px; line-height: 24px; margin: 16px 0 0 0;">If you''d like to continue with Premium features, you can upgrade at any time.</p></div>' WHERE template_key = 'trial_expired';
...
UPDATE email_templates SET html_body = '<div style="margin-bottom: 24px;"><p style="color: #374151; font-size: 16px; line-height: 24px; margin: 0 0 16px 0;">Hi {{userName}},</p><p style="color: #374151; font-size: 16px; line-height: 24px; margin: 0 0 16px 0;">Here is your weekly reflection summary for {{weekStart}} - {{weekEnd}}:</p><div style="background: #F9FAFB; padding: 20px; border-radius: 8px; margin: 16px 0;"><p style="color: #1F2937; font-size: 16px; line-height: 24px; margin: 0 0 16px 0;">{{insights}}</p><p style="color: #6B7280; font-size: 14px; margin: 0;"><strong>Reflections this week:</strong> {{reflectionCount}}</p></div></div>' WHERE template_key = 'weekly_digest';

UPDATE email_templates SET html_body = '<div style="margin-bottom: 24px;"><p style="color: #374151; font-size: 16px; line-height: 24px; margin: 0 0 16px 0;">Hi {{userName}},</p><p style="color: #374151; font-size: 16px; line-height: 24px; margin: 0;">This is a gentle check-in. If you''d like a prompt today, you can open Prompt & Pause whenever you''re ready.</p></div>' WHERE template_key = 'streak_milestone';

UPDATE email_templates SET html_body = '<div style="margin-bottom: 24px;"><p style="color: #374151; font-size: 16px; line-height: 24px; margin: 0 0 16px 0;">Hi {{userName}},</p><p style="color: #374151; font-size: 16px; line-height: 24px; margin: 0;">{{achievementDescription}}</p></div>' WHERE template_key = 'achievement_unlocked';

UPDATE email_templates SET html_body = '<div style="margin-bottom: 24px;"><p style="color: #374151; font-size: 16px; line-height: 24px; margin: 0 0 16px 0;">Hi {{userName}},</p><p style="color: #374151; font-size: 16px; line-height: 24px; margin: 0;">If you''d like a quiet moment to reflect, Prompt & Pause is here whenever you return.</p></div>' WHERE template_key = 'inactivity_reminder';
...
-- Verification Query
-- ============================================================================
-- Run this to verify all templates were updated:
