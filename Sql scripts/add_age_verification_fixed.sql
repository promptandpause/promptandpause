-- ============================================================================
-- ADD AGE VERIFICATION TO PROFILES TABLE (FIXED SYNTAX)
-- ============================================================================
...
BEGIN
    IF NOT EXISTS (
        ...
        WHERE conname = 'valid_country_code' 
...
BEGIN
    IF NOT EXISTS (
        ...
        WHERE policyname = 'Users can view own age verification'
        AND tablename = 'profiles'
    ) THEN
        CREATE POLICY "Users can view own age verification" ON public.profiles
        FOR SELECT USING (auth.uid() = id);
    END IF;
    
    IF NOT EXISTS (
        SELECT 1 FROM pg_policies 
        WHERE policyname = 'Users can update own age verification'
        AND tablename = 'profiles'
    ) THEN
        CREATE POLICY "Users can update own age verification" ON public.profiles
